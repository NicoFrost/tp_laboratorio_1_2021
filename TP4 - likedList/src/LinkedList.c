	...
	if(!(this == NULL || nodeIndex < 0 || nodeIndex > tam)){
		...
		if(node != NULL){
			...
			if(nodeIndex == 0){
				...
				getNode(this, nodeIndex-1)->pNextNode = node;
			}
			this->size++;
			returnAux = 0;
		}
	...
	int returnAux = -1;
		...
		if(node != NULL){ //VERIF NODE
			if(index == 0){
				...
				if(nodeAux != NULL){ //VERIF NODEAUX
					if(index == ll_len(this)){
						nodeAux->pNextNode = NULL;
					} else {
						nodeAux->pNextNode = node->pNextNode;
					}
					returnAux = 0;
					this->size--;
				}
		...
		for(i = 0; i < cant-1;i++){
			for(j = i+1; j < cant;j++){
				if((pFunc(ll_get(this, i), ll_get(this, j)) > 0 && order == 1) || (pFunc(ll_get(this, i), ll_get(this, j)) < 0 && order == 0)){
					pElementAux = ll_get(this, i); // aux = a
					ll_set(this, i, ll_get(this, j));//a = b
					ll_set(this, j, pElementAux);//b = aaux
